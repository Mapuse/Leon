...
/// Draws one 5x7 glyph at 2x scale. The glyph data is authored with the most
/// significant bit as the leftmost column, so bit `col` maps to screen column
/// `GLYPH_W - 1 - col`.
...
/// 5x7 glyphs (7 rows, 5 columns, bit 4 = leftmost column). Only the
...
const FONT: &[(char, [u8; 7])] = &[
    ...
    ('t', [0x04, 0x0E, 0x04, 0x04, 0x04, 0x05, 0x06]),
    ('u', [0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x0F]),
    ('v', [0x11, 0x11, 0x0A, 0x0A, 0x0A, 0x04, 0x04]),
    ('w', [0x11, 0x11, 0x11, 0x15, 0x15, 0x15, 0x0A]),
    ('y', [0x11, 0x11, 0x0A, 0x0A, 0x04, 0x04, 0x08]),
