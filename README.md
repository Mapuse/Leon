...
The Bootloader of **`[Cudane]`**, a Flicker-Free Replace of **`[GRUB]`** and **`[rEFInd]`** Written in **`[Rust]`**, It acquires the **`[BGRT]`** motherboard logo and the **`[GOP]`** frame buffer without ever calling `set_mode`, then chainloads any `\EFI\*.efi` boot entry — its own kernel included — with the screen untouched and zero flicker.
