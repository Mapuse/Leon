//! Command-line argument parsing for the `lbc` command tree.
//!
//! `lbc` uses a custom, table-driven dispatcher instead of a derive CLI so
...
//!
//! ```text
//! lbc -j d                 # short flag + alias for `discover`
//! lbc c get -k timeout     # alias `c` + subcommand + long flag
//! lbc b stage -d ./esp -a arm64
//! ```
...
pub fn parse_from(tokens: Vec<String>) -> Result<Action> {
    ...
    match &node.kind {
        ...
        Kind::Leaf(_) => Ok(Action::Run(node, parsed)),
    }
}
